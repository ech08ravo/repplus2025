      ...
      tags$div(class = "sidebar-section",
        ...
        tags$div(class = "sidebar-content", style = "display: none;",
          ...
          div(style = "display: flex; align-items: center; gap: 6px;",
            checkboxInput("rgrid_zero_na", "0 = N/A", value = FALSE),
            actionButton("info_zero_na", "?", class = "btn-info",
                         style = "width: 18px; height: 18px; padding: 0; font-size: 11px; line-height: 18px; border-radius: 50%; margin-top: -20px;")
          ),
          conditionalPanel(
            condition = "input.info_zero_na % 2 == 1",
            div(class = "info-popup", style = "font-size: 11px;",
              tags$strong("What a 0 in the file means"), tags$br(),
              "Tick this only if 0 marks a construct that ", tags$em("does not apply"),
              " to an element, the way Bezzi (1996) printed his grid. Those cells are read as not rated.",
              tags$br(), tags$br(),
              tags$strong("Leave it unticked for Rep Plus files."), " Rep Plus stores ratings starting at 0, so a 0 there is a real rating - the left pole - and ticking the box would throw those ratings away.",
              tags$br(), tags$br(),
              tags$strong("Where the ratings start is worked out for you."),
              " It is a separate question, decided from the grid itself:",
              tags$ul(style = "margin: 4px 0; padding-left: 16px;",
                tags$li("Ratings that reach the top of the scale (a 5 on a 1-5 scale) are already 1-5 and are left alone."),
                tags$li("Ratings that stop one short (top out at 4) started at 0, so 1 is added to each: 0-4 becomes 1-5."),
                tags$li("If no element sits at either pole, there is nothing to judge from, so the file's own source is used - Rep Plus grids always start at 0.")
              ),
              "Because the two are independent, a grid can use 0 for N/A ", tags$em("and"),
              " start its ratings at either 0 or 1. The message shown after loading reports what was applied.")
          ),
